...
public class PathFinder {
   ...
   */
  public List<IToken> findPath(final IGrid grid, final int xStart, final int yStart, final int xEnd,
      final int yEnd) {
    ...
    for (int mul = 0; possible; mul++) {
      ...
      for (int i = 0; i < sNeigh.length; i++) {
        ...
        possible = true;
        final List<IToken> temp =
            this.directPath(grid, i % 2 == 0 ? end : xStart, i % 2 == 0 ? yStart : end, xEnd, yEnd);
        if (temp == null) {
          continue;
        }
        path.addAll(temp);
  ...
  private List<IToken> lineFree(final IGrid grid, final int xStart, final int yStart, final int end,
    ...
    for (int i = mode ? xStart : yStart; i != end; i += (i < end) ? 1 : -1) {
      if (grid.getTokenAt(mode ? i : xStart, mode ? yStart : i) == null || (grid
          ...
          && grid.getTokenAt(mode ? i : xStart, mode ? yStart : i)
              .getTokenState() != TokenState.CLICKED))
        return null;
   ...
   */
  private List<IToken> directPath(final IGrid grid, final int xStart, final int yStart,
  ...
  private IToken[] getNeighbours(final IGrid grid, final int x, final int y) {
    return new IToken[] {grid.getTokenAt(x + 1, y), grid.getTokenAt(x, y - 1),
        grid.getTokenAt(x - 1, y), grid.getTokenAt(x, y + 1),};
  }
}
